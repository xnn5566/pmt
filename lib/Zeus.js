...
Zeus.getAppConfigs = function (callback){
    var configs = [];
    for (var appName in AppDatabase){
        configs.push(AppDatabase[appName].config);
    }
...
Zeus.killWorkerWithId = function(appName, id, callback){
    ...
    switch (workerDb.status) {
        ...
        case cst.ONLINE_STATUS:
            workerDb.status = cst.STOPPING_STATUS; // stopping
            ...
            setTimeout(function (){
                if(workerDb.status != cst.OFFLINE_STATUS){
                    console.log('Worker \'' + appName + id +  '\' disconnect timeout, begin to kill');
                    worker.kill();
                }
            }, cst.DISCONNECT_TIMEOUT);
            break;
        // offline
...
function workerEventHandle(appName, id, worker){
    ...
    worker.on('disconnect', function() {
        console.log(workerFrefix + ' disconnect');
    });
    ...
    worker.on('exit', function(code, signal) {
        ...
        if (!workerDb.busy && AppDatabase[appName].config.auto_restart == 'true'){
            console.log(workerFrefix + 'auto restart after ' + cst.RESTART_DELAY / 1000 + 's');
            setTimeout(function (){

...
Titan.pingDaemon = function(callback){
    ...
    nativeClient.once('error', function() {
        process.nextTick(function() {
            return callback('Pmt Daemon doesn\'t exist');
        });
    });
...
Titan.killDaemon = function (callback) {
    client.socket.once('end', function(){
        fs.unlinkSync(cst.DAEMON_PRO_PORT);
        eventBusLinked = false; // if event bus linked
        clientConnected = false; // if socket client connected
        daemonIsAlive = false; // if daemon is alive
        console.log('kill Daemon success');
        return callback && callback(null);
    });
    Titan.executeMothed('killDaemon');
}
...
Titan.startApp = function (config, callback){
    Titan.executeMothed('startAppWithCfg', util._extend({}, config), callback);
}
